...
export default function ObjectTypeTable({objectType} : { objectType: ObjectType}) {
            ...
            <tr>
              <th style={{ width: 28, textAlign: 'center', padding: '12px 6px' }}>
                ...
                <th style={{ width: 120, padding: '12px 6px' }}>
                    ...
                    >
                    {objectType.properties[property].title}
                    </Link>
                </th> :
                <th style={{ width: 140, padding: '12px 6px' }}> {objectType.properties[property].title} </th>
                }</>)
            ...
            {[...rows].sort(getComparator(order, 'version')).map((row) => (
              <tr key={row.id}>
                ...
                </td>
                {
                  Object.keys(objectType.properties).map(property => 
                    <td>
                        <Typography level="body-xs">{row[property]}</Typography>
                    </td>
                  )  
                }
                <td>
                  <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                    <Link level="body-xs" component="button" onClick={() => navigate(objectType.path + "/" + row.key)}>
                      Edit
                    </Link>
                    <RowMenu />
                  </Box>
